...
import { SuccessMessage, Error } from './types.js';
...
let cookieOptions: CookieOptions = {
    httpOnly: true, // No JS zone
    secure: true, // only https allowed
...
type APIResponse = Response<SuccessMessage | Error>

async function authorDelete(req: Request, res: APIResponse) {
    ...
    try {
        ...
        let initResult = await initSelect.all(function(err: any, rows: any) {
            return rows;
        });
        ...
        let deleteStatement = await db.prepare(
...
async function bookDelete(req: Request, res: APIResponse) {
...
async function authorGet(req: Request, res: APIResponse) {
    ...
    if (!(Object.keys(req.query).length === 0)) {
        ...
        try {
            ...
            let result = await select.all(function(err:any, rows:any) {
                return rows;
            });
            ...
            return res.status(400).json({ error: `Error with retrieving; code returned by SQLite: ${error?.code}` });
        }
    }
    ...
    try {
        ...
        return res.status(200).json({ error: `Error with retrieving; code returned by SQLite: ${error?.code}`});
    }
}

async function bookGet(req: Request, res: APIResponse) {
    ...
    if (!(Object.keys(req.query).length === 0)) {
        ...
        try {
            ...
            let result:object[] = await select.all(function(err:any, rows:object[]) {
                return rows;
            });
...
async function userEntriesGet(req: Request, res: APIResponse) {
    ...
    try {
        ...
        let bookResult = await selectBooks.all(function(err: any, rows: any) {
            return rows;
        });

        let selectAuthors = await db.prepare('SELECT * FROM authors WHERE user = ?');
        await selectAuthors.bind([username]);
        let authorResult = await selectAuthors.all(function(err: any, rows: any) {
            return rows;
        });
...
async function authorPost(req: Request, res: APIResponse) {
    ...
    try {
        ...
        let insertStatement = await db.prepare(
            'INSERT INTO authors(id, name, bio, user) VALUES (?, ?, ?, ?)'
        );
        await insertStatement.bind(
            [newID, req.body.name, req.body.bio, currentUser]
        );
...
async function bookPost(req: Request, res: APIResponse) {
    // Check body for validity
    ...
    try {
        let currentUser = tokenStorage[req.cookies.token].username;
        let insertStatement = await db.prepare(
            'INSERT INTO books(id, author_id, title, pub_year, genre, user) VALUES (?, ?, ?, ?, ?, ?)'
        );
        await insertStatement.bind(
            [newID, req.body.author_id, req.body.title, req.body.pub_year, req.body.genre, currentUser]
        );
        await insertStatement.run();
        ...
        return res.status(400).json({ error: `Error with inserting; code returned by SQLite: ${error?.code}` });
    }
}

async function bookPut(req: Request, res: APIResponse) {
    ...
    try {
        ...
        return res.status(400).json({ error: `Error with updating; code returned by SQLite: ${error?.code}` });
    }
}
// Taken from CST480 activity 5 on authentication
async function login(req: Request, res: APIResponse) {
    ...
    try {
        ...
        let result = await select.all(function(err: any, rows: object[]) {
            return rows;
        });
        if (result.length === 0) {
...
async function logout(req: Request, res: APIResponse) {
